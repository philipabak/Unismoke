...
class AdminProducts extends Admin {
    ...
    public function approve_review()
    {
        ...
        $results = \CI::Products()->approve_review($id, $status);
    ...
    public function delete_review()
    {
        ...
        $results = \CI::Products()->delete_review($id);
        echo json_encode($results);
    }
    ...
    public function form($id = false, $duplicate = false)
    {
        ...
        if ($id)
        {
            ...
            $data['productReviews'] = \CI::Products()->getProductReviews($id);
            ...
            $data['manufacturer_data'] = $product->manufacturer_data;
            ...
            if(!\CI::input()->post('submit'))
            {
                ...
                $data['related_swatches'] = $product->related_swatches;

                $data['images'] = (array)json_decode($product->images);

                //get accessory_products data from products_accessories table
                $data['accessory_products'] = $product->accessory_products;
                //--end
            }
        }
        ...
        //if $data['accessory_products'] is not an array, make it one.
        if(!is_array($data['accessory_products'])) {
            $data['accessory_products'] = [];
        }

        //if $data['related_swatches'] is not an array, make it one.
        if(!is_array($data['related_swatches'])) {
            $data['related_swatches'] = [];
        }
        ...
        */
        ...
        if(\CI::input()->post('submit'))
        {
            ...
            $data['accessory_products'] = \CI::input()->post('accessory_products');
            $data['related_swatches'] = \CI::input()->post('related_swatches');
        ...
        else
        {
            ...
            $save['id'] = $id;
            $save['manufacturer'] = \CI::input()->post('manufacturer');
            ...
            $primary_images = \CI::input()->post('primary_images');
            ...
            foreach($post_images as $key => $image){  // Removing all the false colors before saving to DB
                foreach($image as $index => $value){
                    if( substr($index,0,-2) == 'color' || $index == 'color'){
                        foreach($value as $property => $property_value){
                            if($property == 'selected' && $property_value == "false") {
                                unset($post_images[$key][$index]);  // removing all the irrelevant colors.
                            }elseif($property == 'selected' && $property_value == "true"){
                                $post_images[$key]['color'] = $post_images[$key][$index];
                                if($index != "color") {  // If Index different from Color(we only want to delete color_0 _1 ... )
                                    unset($post_images[$key][$index]);
                                }
                                if(is_array($primary_images)) {  // Making sure we got one image set as primary.
                                    if (in_array($key, $primary_images)) {  // This is a main image of the color swatch.
                                        $image_name_with_dir = $image['filename'];
                                        $image_name = substr($image_name_with_dir, strrpos($image_name_with_dir, '/') + 1);
                                        $post_images[$key]['color']['primary_image'] = $image_name;
                                    }
                                }
                            }
                        }
            ...
            if(\CI::input()->post('related_swatches'))
            {
                $save['related_swatches'] = json_encode(\CI::input()->post('related_swatches'));
            }
            else
            {
                $save['related_swatches'] = '';
            }

            //get accessory_products
            if(\CI::input()->post('accessory_products'))
            {
                $save_accessory['accessory_products'] = \CI::input()->post('accessory_products');
            }
            else
            {
                $save_accessory['accessory_products'] = [];
            }
            ...
            \CI::AdminMedia()->index(DIR_TMP_UPLOAD);  // Resize all images and then save the product.
            // save product
            $product_id = \CI::Products()->save($save, $options, $categories, $save_accessory);
    ...
    public function product_image_form()
    {
        ...
        $data['error']  = false;
        \CI::load()->library('adminMedia');
        $data = \CI::AdminMedia()->index();
        $this->partial('iframe/header', $data);
        $this->partial('iframe/product_image_uploader', $data);
        $this->partial('iframe/footer', $data);
